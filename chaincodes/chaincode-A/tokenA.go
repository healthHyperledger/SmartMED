...
func (clientdid *mycc) ReadAttributesOfCaller(stub shim.ChaincodeStubInterface) peer.Response {
	...
	// 4. Get the attr value for "type of user"
	attrValue, flag, _ := cid.GetAttributeValue(stub, "type.role")
	if !flag {
		attrValue = "NOT SET"
	}
	// Format and add the attribute to JSON
	jsonResult += "," + SetJSONNV("userType", attrValue)

	// 5. Get the attr value for "department"
	attrValue, flag, _ = cid.GetAttributeValue(stub, "associatedPatients")
	if !flag {
		attrValue = "NOT SET"
	}
	// Format and add the attribute to JSON
	jsonResult += "," + SetJSONNV("associatedPatients", attrValue)
...
func (clientdid *mycc) ApproveWrite(stub shim.ChaincodeStubInterface, writeUser string) peer.Response {
	attrValue, _, _ := cid.GetAttributeValue(stub, "type.role")

	if attrValue == "requestor" {
...
func (clientdid *mycc) ApproveElevate(stub shim.ChaincodeStubInterface, writeUser string, levelReq string) peer.Response {
	attrValue, _, _ := cid.GetAttributeValue(stub, "type.role")
	response := shim.Error("nil")
	if attrValue == "dataOwner" {
		stub.PutState(writeUser, []byte("Write Auth."))
		response = shim.Success([]byte(levelReq))

	} else if attrValue == "requestor" {
		response = shim.Error("You're not authorized.")
	}
...
func (clientdid *mycc) readRecord(stub shim.ChaincodeStubInterface, dataOwner string, levelReq string) peer.Response {
	key := "associatedPatients." + dataOwner
	attrValue, _, _ := cid.GetAttributeValue(stub, key)
...
func (clientdid *mycc) writeToRecord(stub shim.ChaincodeStubInterface, caller string, dataOwner string, writeLevel string) peer.Response {
...
func (clientdid *mycc) elevatePriv(stub shim.ChaincodeStubInterface, caller string, dataOwner string, newLevel string) peer.Response {
...
func main() {
	fmt.Println("Directory creation in process.")
	appFs := afero.NewOsFs()
	appFs.Mkdir("test1", 1)
