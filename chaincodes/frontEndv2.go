...
func (clientdid *mycc) Invoke(stub shim.ChaincodeStubInterface) peer.Response {
	...
	} else if funcName == "logs" {
		return clientdid.ReadLogs(stub, args[0])
	}

	return shim.Error("Wrong function call.")
}
func (clientdid *mycc) ReadLogs(stub shim.ChaincodeStubInterface, dataOwner string) peer.Response {
