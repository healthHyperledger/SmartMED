...
func (clientdid *mycc) Invoke(stub shim.ChaincodeStubInterface) peer.Response {
	...
	if funcName == "readattrs" {
		...
		return clientdid.freadRecord(stub, args[0], args[1])
	} else if funcName == "fwrite" {
		return clientdid.fwriteToRecord(stub, args[0], args[1])
	}
...
func (clientdid *mycc) freadRecord(stub shim.ChaincodeStubInterface, dataOwner string, levelReq string) peer.Response {
...
func (clientdid *mycc) fwriteToRecord(stub shim.ChaincodeStubInterface, dataOwner string, writeLevel string) peer.Response {
	caller, _, err := cid.GetAttributeValue(stub, "hf.EnrollmentID")
	//caller, err := cid.GetID(stub)
	if err != nil {
		return shim.Error("callerID malfunctioned")
